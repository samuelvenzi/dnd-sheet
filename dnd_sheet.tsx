...
const defSkills = () => {
  const o = {};
  SKILLS.forEach((s) => {
    o[s.name] = { proficient: false, expertise: false };
  });
  return o;
};
const defCond = () => {
  const o = { exhaustion: 0 };
...
const defSpells = () => {
  const o = {};
  for (let i = 0; i <= 9; i++) o[i] = [];
  return o;
};
const defSlots = () => {
  const o = {};
  for (let i = 1; i <= 9; i++) o[i] = { total: 0, used: 0 };
  return o;
};
...
const DEFAULT = {
  ...
  combat: {
    ...
    tempHp: 0,
    hdCurrent: 1,
    dsSuc: [false, false, false],
    dsFail: [false, false, false],
  },
  conditions: defCond(),
  attacks: [],
  features: [],
...
export default function Sheet() {
  ...
  const [spellOpen, setSpellOpen] = useState({
    0: true,
    1: true,
    2: false,
    3: false,
  ...
  const skillTotal = (n) => {
    const s = C.skills[n];
    const sk = SKILLS.find((x) => x.name === n);
    const base = mods[sk.ab];
    if (s.proficient) return base + PB + (s.expertise ? PB : 0);
    return base + (jot ? Math.floor(PB / 2) : 0);
  };
  ...
  const init =
    C.combat.initOverride !== null ? C.combat.initOverride : mods.dex;
  const totalWt = C.equipment.items.reduce(
    (s, i) => s + (parseFloat(i.weight) || 0) * (parseInt(i.qty) || 1),
    0,
  );
  const carrying = C.abilities.str * 15;
  const encSt =
    totalWt > C.abilities.str * 10
      ? "Heavily Encumbered"
      : totalWt > C.abilities.str * 5
        ? "Encumbered"
        : "Normal";
  const totalGP =
    C.equipment.cp / 100 +
  ...
  const exportJSON = () => {
    const nm = C.identity.name || "character";
    const b = new Blob([JSON.stringify(C, null, 2)], {
      type: "application/json",
    });
    const u = URL.createObjectURL(b);
    const a = document.createElement("a");
    a.href = u;
    a.download = `${nm.replace(/\s+/g, "_")}.json`;
    a.click();
  ...
  const applyHp = (dmg) => {
    const d = parseInt(hpDelta) || 0;
    const nv = dmg
      ? Math.max(0, C.combat.currentHp - d)
      : Math.min(
          C.combat.maxHp + (C.combat.tempHp || 0),
          C.combat.currentHp + d,
        );
    upd("combat.currentHp", nv);
    setHpOpen(false);
    setHpDelta("");
  };
    ...
    >
                ...
                >
                  ...
                  >
                    ...
                    <input
                      type="checkbox"
                      checked={sv}
                      onChange={(e) => upd(`saves.${ab}`, e.target.checked)}
      ...
      <div style={{ ...card, alignSelf: "start" }}>
        ...
        >
          ...
          {jot && (
            ...
            >
              Jack of All Trades +{Math.floor(PB / 2)}
            </span>
          )}
        </div>
        ...
        >
          {SKILLS.map((sk) => {
            const sv = C.skills[sk.name];
            const tot = skillTotal(sk.name);
            return (
              <div
                key={sk.name}
                style={{
              ...
              >
                <input
                  type="checkbox"
                  checked={sv.proficient}
                  onChange={(e) =>
                    setC((p) =>
                      deepSet(
                        p,
                        `skills.${sk.name}.proficient`,
                        e.target.checked,
                      ),
                    )
                  }
                ...
                <input
                  type="checkbox"
                  checked={sv.expertise}
                  onChange={(e) =>
                    setC((p) =>
                      deepSet(
                        p,
                        `skills.${sk.name}.expertise`,
                        e.target.checked,
                      ),
                    )
                  }
      ...
      >
        ...
        <div style={card}>
          ...
          <textarea
            value={C.profLangs}
            onChange={(e) => upd("profLangs", e.target.value)}
            rows={3}
            style={txtArea}
      ...
      <div style={card}>
        ...
        >
          ...
          <StatBox label="Initiative">
            ...
            <input
              ...
              style={{ ...numInp, fontSize: 11, marginTop: 4, width: 60 }}
              placeholder="override"
            />
          </StatBox>
          <StatBox label="Speed (ft)">
            <input
              type="number"
              min={0}
          ...
          <StatBox label="Current HP" accent>
            <div
              style={{
                fontSize: 22,
                fontWeight: 800,
                color: isDying ? T.danger : T.accent,
                lineHeight: 1,
                ...(isDying ? { animation: "pulse 1s infinite" } : {}),
              }}
            >
              {C.combat.currentHp}
            </div>
            <button
              ...
              style={{ ...btn(), marginTop: 4, fontSize: 11 }}
            >
              +/−
            </button>
          </StatBox>
          <StatBox label="Temp HP">
            <input
              ...
              onChange={(e) =>
                upd("combat.tempHp", parseInt(e.target.value) || 0)
              }
              style={{
                ...numInp,
                fontSize: 22,
        ...
        >
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input
                ...
                min={0}
                max={lvl}
                value={C.combat.hdCurrent}
                onChange={(e) =>
                  upd(
                    "combat.hdCurrent",
                    Math.max(0, Math.min(lvl, parseInt(e.target.value) || 0)),
                  )
                }
                style={{ ...numInp, width: 50 }}
              />
            </div>
          </div>
          <div
          ...
          >
            ...
            >
              <Heart size={13} color={T.success} />
              <span style={{ fontSize: 12, color: T.sub }}>Successes</span>
              {C.combat.dsSuc.map((v, i) => (
                <input
                  key={i}
                  type="checkbox"
                  checked={v}
                  onChange={(e) => {
                    const a = [...C.combat.dsSuc];
                    a[i] = e.target.checked;
                    upd("combat.dsSuc", a);
                  }}
                  style={{ accentColor: T.success, width: 15, height: 15 }}
                />
              ))}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <Skull size={13} color={T.danger} />
              <span style={{ fontSize: 12, color: T.sub }}>Failures</span>
              {C.combat.dsFail.map((v, i) => (
                <input
                  key={i}
                  type="checkbox"
                  checked={v}
                  onChange={(e) => {
                    const a = [...C.combat.dsFail];
                    a[i] = e.target.checked;
                    upd("combat.dsFail", a);
                  }}
                  style={{ accentColor: T.danger, width: 15, height: 15 }}
                />
              ))}
            </div>
          </div>
            ...
            >
              ...
              >
                <input
                  ...
                  onChange={(e) => {
                    const arr = [...C.attacks];
                    arr[i] = { ...arr[i], bonus: e.target.value };
                    upd("attacks", arr);
                  }}
                  style={{ ...numInp }}
                  placeholder="+5"
                ...
                <input
                  ...
                  onChange={(e) => {
                    const arr = [...C.attacks];
                    arr[i] = { ...arr[i], damage: e.target.value };
                    upd("attacks", arr);
                  }}
                  style={inp}
      ...
      <div style={card}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {CONDITIONS.map((c) => (
            <button
              key={c}
              onClick={() => upd(`conditions.${c}`, !C.conditions[c])}
              style={tag(C.conditions[c])}
            >
              {c}
            </button>
          ))}
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <select
              value={C.conditions.exhaustion}
              onChange={(e) =>
                upd("conditions.exhaustion", parseInt(e.target.value))
              }
  ...
  const renderSpells = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        <div>
          <div style={label}>Spellcasting Class</div>
          <input
            value={C.spellcasting.cls}
            onChange={(e) => upd("spellcasting.cls", e.target.value)}
            style={inp}
            placeholder="Class"
          />
        </div>
        <div>
          <div style={label}>Spellcasting Ability</div>
          <select
            value={C.spellcasting.ability}
            onChange={(e) => upd("spellcasting.ability", e.target.value)}
            style={inp}
          >
                ...
                >
                  {Array.from({ length: sl.total }, (_, i) => (
                    <button
                      key={i}
                      onClick={() => {
                        const nu = i < sl.used ? i : i + 1;
                        setC((p) =>
                          deepSet(p, `spellcasting.slots.${l}.used`, nu),
                        );
                      }}
                      ...
                      style={{
                        ...
                        height: 10,
                        borderRadius: "50%",
                        border: `1px solid ${i < sl.used ? T.muted : T.accent}`,
                        background: i < sl.used ? T.pipEmpty : T.pip,
                        cursor: "pointer",
                        padding: 0,
                        transition: "all 0.15s",
                      }}
        ...
        return (
          <div key={lvl} style={card}>
            <div
              ...
              onClick={() => setSpellOpen((p) => ({ ...p, [lvl]: !open }))}
            >
              <div style={label}>
            ...
            {open && (
              ...
              >
                {spells.map((sp, i) => {
                  ...
                  const setSpArr = (arr) =>
                    setC((p) => deepSet(p, `spells.${lvl}`, arr));
                  const upSp = (f, v) => {
                    const a = [...C.spells[lvl]];
                    a[i] = { ...a[i], [f]: v };
                    setSpArr(a);
                  };
                  const upSpC = (c, v) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        style={{
                          display: "grid",
                          gridTemplateColumns: "1fr minmax(100px,160px)",
                          gap: 8,
                          marginBottom: 8,
                        }}
                      >
                          ...
                          {lvl > 0 && (
                            ...
                            >
                              <input
                                ...
                                onChange={(e) =>
                                  upSp("prepared", e.target.checked)
                                }
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            setSpellDesc((p) => ({ ...p, [dk]: !dOpen }))
                          }
                          style={{ ...btn(), marginLeft: "auto", fontSize: 11 }}
                        >
                          {dOpen ? "▲ Hide" : "▼ Desc"}
                        </button>
                        <button
      ...
      <div style={card}>
        ...
        >
          {[
            ["cp", "CP", "Copper", "#b45309"],
            ["sp", "SP", "Silver", "#64748b"],
            ["ep", "EP", "Electrum", "#3b82f6"],
            ["gp", "GP", "Gold", "#d97706"],
            ["pp", "PP", "Platinum", "#8b5cf6"],
          ].map(([k, abbr, label_, cl]) => (
            <div
              key={k}
              style={{
                textAlign: "center",
            ...
            >
              <div style={{ fontSize: 13, fontWeight: 700, color: cl }}>
                {abbr}
              </div>
        ...
        >
          GP equivalent:{" "}
          <strong style={{ color: T.accent }}>{totalGP.toFixed(2)} gp</strong>
        </div>
      </div>
      <div style={card}>
        <div
                  ...
                  >
                    ...
                    <td style={{ padding: "4px 4px" }}>
                      <input
                        ...
                        value={it.qty || 1}
                        onChange={(e) =>
                          upIt("qty", parseInt(e.target.value) || 1)
                        }
                        style={{ ...numInp, width: 50 }}
                      />
                    </td>
        ...
        >
          <span>
            Total weight:{" "}
            <strong style={{ color: T.text }}>{totalWt.toFixed(1)} lb</strong>
          </span>
          <span>
            Capacity: <strong style={{ color: T.text }}>{carrying} lb</strong>
          </span>
          <span
            style={{
              color: encSt !== "Normal" ? T.danger : T.success,
              fontWeight: 600,
            }}
          >
            {encSt}
  ...
  const renderNotes = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        >
          {[
            ...
            <div key={k}>
              <div style={label}>{l}</div>
              <input
                value={C.notes.appearance[k] || ""}
                onChange={(e) =>
                  setC((p) =>
                    deepSet(p, `notes.appearance.${k}`, e.target.value),
                  )
                }
                style={inp}
                placeholder={l}
      ...
      <div style={card}>
        <div style={label}>Character Backstory</div>
        <textarea
          value={C.notes.backstory}
          onChange={(e) => upd("notes.backstory", e.target.value)}
          rows={8}
          style={txtArea}
      ...
      >
        <div style={card}>
          <div style={label}>Allies & Organizations</div>
          <textarea
            value={C.notes.allies}
            onChange={(e) => upd("notes.allies", e.target.value)}
            rows={6}
            style={txtArea}
            placeholder="Allies, factions, organizations..."
          />
        </div>
        <div style={card}>
          <div style={label}>Treasure</div>
          <textarea
            value={C.notes.treasure}
            onChange={(e) => upd("notes.treasure", e.target.value)}
            rows={6}
            style={txtArea}
            placeholder="Special items, non-tracked wealth..."
          />
        </div>
      </div>
      <div style={card}>
        ...
        <textarea
          value={C.notes.additional}
          onChange={(e) => upd("notes.additional", e.target.value)}
          rows={6}
          style={txtArea}
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            ...
            >
              <button
                onClick={() => {
                  setC(JSON.parse(JSON.stringify(DEFAULT)));
                  setResetOpen(false);
                }}
                style={{
                  ...btn("danger"),
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          >
            <div style={{ gridColumn: "span 2" }}>
              ...
              <input
                ...
                onChange={(e) => upd("identity.name", e.target.value)}
                style={{
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <select
                  value={C.identity.cls}
                  onChange={(e) => upd("identity.cls", e.target.value)}
                  style={{ ...inp, flex: 1 }}
                >
            ...
            <div>
              ...
              <select
                value={C.identity.race}
                onChange={(e) => upd("identity.race", e.target.value)}
                style={inp}
              >
            ...
            <div>
              ...
              <select
                value={C.identity.background}
                onChange={(e) => upd("identity.background", e.target.value)}
                style={inp}
              >
            ...
            <div>
              ...
              <select
                value={C.identity.alignment}
                onChange={(e) => upd("identity.alignment", e.target.value)}
                style={inp}
              >
          ...
          >
              ...
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <input
                    ...
                    style={{ ...numInp, width: 80, fontSize: 13 }}
                  />
                  <span style={{ fontSize: 11, color: T.muted }}>
            ...
            >
              ...
              <a
                href="https://github.com/samuelvenzi/dnd-sheet"
                target="_blank"
                rel="noreferrer"
      ...
      >
        Unofficial fan tool — not affiliated with, endorsed by, or connected to
        Wizards of the Coast. Dungeons &amp; Dragons is a trademark of Wizards
        of the Coast LLC. Used under the{" "}
        <a
        ...
        .{" · "}
        <a
          href="https://github.com/samuelvenzi/dnd-sheet#readme"
          target="_blank"
          rel="noreferrer"
          style={{ color: T.sub }}
        >
          How to create your character JSON
        </a>
      </div>
    </div>
  );
